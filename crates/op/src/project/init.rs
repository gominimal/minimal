...
impl ProjectOp for InitProject {
    ...
    fn run<E: ProjectEnv>(&mut self, env: &mut E) -> Result<InitPlan, Error> {
        ...
        let (origin, link, toml_path) = match existing {
            ...
            None => {
                env.update_checkout(DEFAULT_PKGS)?;
...
mod tests {
    ...
    #[test]
    fn match_stack_emits_packages_in_a_stable_order() {
        // Guards the determinism of `min init`: the detected build and runtime
        // package lists must come out in a fixed order every run, so the written
        // `minimal.toml` is byte-identical across invocations. A single-entry
        // list cannot catch an unordered collection sneaking into this path, so
        // this exercises several matching packages at once.
        let dir = TempDir::new().unwrap();
        for file in [
            "Cargo.toml",
            "build.rs",
        ...
        let stack = Stack {
            ...
            matches_project_if_any: Some(vec![StackMatcher {
                file_regexes: [("Cargo.toml".to_string(), "*".to_string())]
                    ...
                    .collect(),
                build_package_matchers: [
                    ("cc".to_string(), vec![exists("build.rs")]),
                    ("pkg-config".to_string(), vec![exists("Cargo.toml")]),
                ]
                .into_iter()
                .collect(),
                runtime_package_matchers: [
                    ("openssl".to_string(), vec![exists("openssl.pc")]),
                    ("zlib".to_string(), vec![exists("zlib.pc")]),
                    ("sqlite".to_string(), vec![exists("sqlite.pc")]),
                ]
                .into_iter()
                .collect(),
                ..Default::default()
        ...
        assert_eq!(build, vec!["cc".to_string(), "pkg-config".to_string()]);
        assert_eq!(
            runtime,
            vec![
                "openssl".to_string(),
