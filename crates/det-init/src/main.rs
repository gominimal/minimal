    ...
    if egress.is_some() {
        match dns_upstream.as_deref() {
            ...
            Some(up) => match install_local_resolver() {
                ...
                Err(e) => {
                    tracing::warn!(error = %e, "could not repoint resolver; DNS names may go unobserved")
                }
            },
        }
    }
...
fn stage_tripwire() {
    ...
    match std::fs::File::create(TRIPWIRE) {
        ...
        Err(e) => {
            tracing::warn!(path = TRIPWIRE, error = %e, "could not stage tripwire (verdict will cap at REVIEW)")
        }
    }
}
...
fn load_sample_command() -> Result<String, MainError> {
    ...
    let path = std::env::var("DET_SAMPLE_FILE").unwrap_or_else(|_| SAMPLE_FILE_DEFAULT.to_string());
    let cmd = std::fs::read_to_string(&path)
        .map_err(|e| MainError::IO(e, "reading the sample command file"))?;
    let cmd = cmd.trim();
    if cmd.is_empty() {
        return Err(MainError::Other(format!(
...
fn run_as_nobody(cmd: &str) -> std::io::Result<std::process::ExitStatus> {
    if std::path::Path::new(DROPPRIV).is_file() {
        ...
        tracing::warn!(
            path = DROPPRIV,
            "droppriv missing; running check without a uid drop"
        );
...
fn nobody_can(cmd: &str) -> bool {
    run_as_nobody(cmd).map(|s| s.success()).unwrap_or(false)
}
...
async fn drain_monitor(monitor: &mut std::process::Child, mon_pid: u32) {
    ...
    if let Ok(Some(status)) = monitor.try_wait() {
        tracing::warn!(
            pid = mon_pid,
            ?status,
            "observer already exited before drain"
        );
        return;
    }
    ...
    for attempt in 0..DRAIN_ATTEMPTS {
        ...
        if attempt + 1 == DRAIN_ATTEMPTS {
            tracing::warn!(
                pid = mon_pid,
                "observer did not exit within 5s of SIGTERM; SIGKILL"
            );
            let _ = monitor.kill();
            let _ = monitor.wait();
            return;
