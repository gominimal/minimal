//! Shared gvproxy-switch primitives used by both `minimald` (the in-guest /
//! native switch client + IP allocator) and `minvmd` (the host switch
//! supervisor). Kept here, in the crate both already depend on, so the subnet
//! arithmetic, MAC derivation, wire constants, and gvproxy `-config` rendering
//! have a single definition rather than drifting copies.
