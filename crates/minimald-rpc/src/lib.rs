...
/// zstd-compressed tar archive of its diagnostic bundle and closes. Errors
/// hit before streaming starts are relayed over extended-data stream 1, so a
/// client that reads zero payload bytes should surface the extended data as
/// the failure reason.
///
...
pub struct DiagBundleRequest {
    /// Per-log-file tail cap in bytes; `0` means the daemon's default.
    #[serde(default)]
...
mod tests {
    ...
    /// An empty request body must decode with the documented defaults so an
    /// older client (or a bare `{}` probe) still gets a full bundle.
    #[test]
    ...
    fn diag_bundle_request_round_trips() {
        let req = DiagBundleRequest {
            log_tail_bytes: 1024,
            include_state_listing: false,
        };
