...
mod tests {
    ...
    /// vocabulary carries no RPC that asks for one and no reply carries the
    /// retired mTLS reverse proxy's status field.
    ///
    /// The needles are assembled from fragments rather than written out, so
    /// this test's own text is not what the source scan finds.
    #[test]
    fn issue_client_cert_absent_from_wire() {
        let source = std::fs::read_to_string(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/lib.rs"),
        )
        .expect("this crate reads its own source");
        let retired_field = ["mtls", "_proxy_unavailable"].concat();
        for retired in [["Issue", "Client", "Cert"].concat(), retired_field.clone()] {
            assert!(
                !source.contains(&retired),
                "{retired} is retired but still declared in the wire contract"
            );
        }

        // The two replies that carried the mTLS proxy's status say nothing
        // about a certificate or a proxy now, even fully populated.
        ...
        for json in [
            ...
            serde_json_lenient::to_string(&created).expect("serializes"),
        ] {
            for retired in ["cert", "mtls"] {
                assert!(
                    !json.contains(retired),
                    "a reply still names {retired}: {json}"
                );
            }
        }

        // A daemon that still sends the retired field is a skew the version
        // gate reports, not a decode failure: the field is dropped.
        let legacy_reply = format!(
            r#"{{"sessions":[],"daemon_version":"0.5.0","{retired_field}":"port 7655 is held"}}"#
        );
        let legacy: ListSessionsResponse = serde_json_lenient::from_str(&legacy_reply)
            .expect("a reply from a daemon that predates the removal must still decode");
