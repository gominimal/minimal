...
/// keys and bound to `box_id`, recording each mint with the proxy
...
async fn mint_store_references<S: bep::KeyStore>(
    ...
    for planned in references.iter().filter(|planned| planned.prompt) {
        ...
        let question = format!(
            "Inject the {reference} into requests from box `{}` to {}?",
            bep::audit::box_name(box_id),
    ...
    let admitted: Vec<sessions::AdmittedReference<'_>> = references
        ...
        .collect();
    mint_store_handles(key_store, identity, control, box_id, host, &admitted, now).await
}

/// Delivers the box spec's credentials into the composition of the session
/// `box_id` names: mints each grant's member from the held sign-in and each
...
///
/// Runs once the session exists, because the sealed context binds the box's
/// subject — its name and the id of this creation ([`box_subject`]) — and
/// neither is final before `CreateSession`.
async fn deliver_box_grants(
    ...
    if !plan.grants.is_empty() {
        ...
        sealed.extend(
            mint_grants(&store, identity, &control, box_id, &host, &plan.grants, now).await?,
        );
    }
...
pub(crate) async fn activate_session(
    ...
    if let Some(plan) = &grant_plan {
        let box_id = box_subject(config.name.as_deref().unwrap_or_default(), id);
        match deliver_box_grants(global, &abs_path, &box_id, plan).await {
            Ok(delivery) => {
...
pub(crate) async fn destroy_session(
    ...
    if let Some(box_name) = name {
        revoke_box_values(control, &box_subject(box_name, id)).await;
    }

    Ok(())
}

/// The subject a box is known by to the proxy: its name and the id of this
/// creation of it, so a box made again under the same name is a different box,
/// out of reach of the revocation of the one before (BEP-043).
pub(crate) fn box_subject(box_name: &str, id: sessions::SessionId) -> String {
    bep::audit::box_subject(box_name, &id.to_string())
}

/// Refuses every sealed value naming `box_id` from here on: the box is gone,
...
///
/// The box a sealed context names is the session's [`box_subject`] — what the
/// mint bound the member to — so a session that never had a name has no value
/// to revoke.
///
...
async fn revoke_box_values(control: &std::path::Path, box_id: &str) {
    let submission = bep::Submission::Audit(bep::box_revocation_event(box_id));
    match crate::auth::submit_audit(control, &submission).await {
        ...
        Err(error) => {
            eprintln!("warning: the proxy did not record the revocation for {box_id}: {error:#}")
        }
    }
}
...
mod tests {
    ...
    async fn activate_mints_and_hands_sealed_value_to_composition() {
        ...
        // Recorded before it was sealed: the member carries its record's place.
    ...
    async fn activate_mints_a_handle_for_a_reference_and_delivers_it() {
        ...
        // The handle's mint is recorded, as a member's is, naming the box and
        // the stored value's identifier; no value of any kind reaches the log.
        let log = std::fs::read_to_string(&audit).unwrap();
        ...
        assert_eq!(records[0].kind, bep::Kind::Mint);
        assert_eq!(records[0].sub, "web");
        assert_eq!(records[0].authority, "api.anthropic.com:443");
        assert_eq!(records[0].credential, "keychain:anthropic-api-key");
        assert!(!log.contains(bep::mint::HANDLE_PREFIX), "{log}");
        assert!(!log.contains(bep::seal::PREFIX), "{log}");
    }
    ...
    /// no other box, not even one made again under the same name — and a proxy
    /// that is not there does not turn the removal into a failure.
    #[tokio::test]
    async fn box_removal_sends_box_revocation() {
        ...
        store
            ...
            .unwrap();
        let keys = Keys::open(MemoryStore::new()).unwrap();
        let creation = |id: &str| box_subject("web", sessions::SessionId::parse_str(id).unwrap());
        let first = creation("0199a4c2-7d1e-7a3b-9f40-2c5e8b1d6a01");
        let again = creation("0199a4d7-1b2c-7e4f-8a90-3d6f9c2e7b02");
        assert_ne!(first, again);
        assert_eq!(bep::audit::box_name(&first), "web");
        let mint_for = |box_id: String| {
            let (store, keys, socket) = (&store, &keys, &socket);
            async move {
                let sealed = mint_grants(
                    store,
                    &keys.public_identity(),
                ...
                .await
                .unwrap();
                bep::unseal(keys, sealed[0].1.as_str())
                    .unwrap()
                    .context
                    .mint_position
            }
        ...
        let minted = mint_for(first.clone()).await;
        assert!(revocations.lock().unwrap().is_empty());

        revoke_box_values(&socket, &first).await;
        let (removed, other_box, module) = {
            let in_force = revocations.lock().unwrap();
            (
                in_force.covers_box(&first, minted),
                in_force.covers_box("api", minted),
        ...
        // The same name again is a new box: the removal does not reach it.
        let minted_again = mint_for(again.clone()).await;
        assert!(!revocations.lock().unwrap().covers_box(&again, minted_again));

        // The record the proxy appended for it: a revocation naming the box's
        // creation, chained onto the mint, and no token in the log.
        let log = std::fs::read_to_string(&audit).unwrap();
        let records: Vec<bep::Record> = log
            .lines()
            .map(|line| serde_json_lenient::from_str(line).unwrap())
        ...
        assert_eq!(records[1].sub, first);
        assert_eq!(records[1].previous_hash, records[0].line_hash());
        assert_eq!(records[2].sub, again);
        assert!(!log.contains(TOKEN), "{log}");

        // No proxy listening: a warning, and nothing appended.
        revoke_box_values(&dir.path().join("absent.sock"), &again).await;
        assert_eq!(
