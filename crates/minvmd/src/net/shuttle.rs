...
/// `krun_add_vsock_port2(.., listen = false)`. Shared with `minimald` via
/// `minimald-rpc` so the guest and host agree on the port.
pub use minimald_rpc::switch::VSOCK_GVPROXY_SHUTTLE_PORT;
