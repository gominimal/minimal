...
// gvproxy-switch primitives are shared with `minimald` via `minimald-rpc`.
// Re-exported so `minvmd::net::{SwitchSubnet, MacAddr, …}` keeps working.
pub use minimald_rpc::switch::{MacAddr, SwitchSubnet, render_gvproxy_config};
