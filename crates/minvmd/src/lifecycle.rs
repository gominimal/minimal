//! Pure lifecycle state machine for `minvmd` (R4.7).
//!
...
/// The lifecycle states of the `minvmd` daemon.
...
pub enum Lifecycle {
    ...
    Stopped,
    /// Daemon is in the process of starting (VM booting).
    Starting,
    /// Daemon is running and the VM is up.
    Running,
...
pub enum Action {
    ...
    Start,
    /// VM is confirmed up; `Starting → Running`.
    MarkRunning,
