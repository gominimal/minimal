...
fn run_macos() -> Result<()> {
    ...
    // vsock bridge.
    let mut cfg = VmConfig::new(2, 1024, kernel, rootfs, exec.clone()).with_data_disk(data_disk);
    // SPIKE: MINVMD_INITRAMFS boots an initramfs (minimald as /init) instead of
    // the block root.
    if let Some(initramfs) = std::env::var_os("MINVMD_INITRAMFS") {
        cfg = cfg.with_initramfs(std::path::PathBuf::from(initramfs));
    }
    cfg.apply(&mut ctx)
