...
fn init_tracing() -> Result<Option<tracing_appender::non_blocking::WorkerGuard>> {
    ...
    let (writer, guard) = tracing_appender::non_blocking::NonBlockingBuilder::default()
        ...
        .finish(appender);
    tracing_subscriber::registry()
        .with(json_file_layer(writer, "minvmd"))
        .with(filter)
        .init();
    tracing::info!(
...
/// The JSON-lines layer for file logs: flat records (span fields flattened
/// to the top level, so `trace_id`/`span_id` are toplevel keys OTLP-mapped
/// consumers can copy), plus static resource fields named by the OTEL
/// semantic conventions. Console output stays human-format; this is for
/// files `min bug` collects and tools parse.
...
mod json_log_shape {
    ...
    fn file_lines_are_flat_json_with_resource_fields_and_span_ids() {
        ...
        let (writer, guard) = tracing_appender::non_blocking::NonBlockingBuilder::default()
            ...
            .finish(buf.clone());
        let subscriber =
            tracing_subscriber::registry().with(super::json_file_layer(writer, "minvmd"));
        ...
        assert_eq!(json["service.name"], "minvmd");
