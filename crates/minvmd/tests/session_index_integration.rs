...
async fn create_session(uds: &Path, name: &str) -> SessionId {
    // A freshly-created session is `Pending` (its loadout is not composed
    // until a later `ConfigureLoadout`), but `ListSessions` returns every
    // record regardless of status, so the poller sees and indexes it without
    // the full upload+configure dance.
    ...
    oneshot::<minimald_rpc::CreateSession>(uds, req)
        .await
        .ok()
