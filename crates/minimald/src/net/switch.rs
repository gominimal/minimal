...
impl SessionGate {
    ...
    pub fn for_session(
        ...
        Self {
            ...
            egress: compiled_egress(Some(policy), subnet),
        }
    }
...
/// another through the proxy (NET-071).
#[must_use]
...
pub fn direct_connection_verdict(
    ...
    egress::verdict(&tcp_frame_summary(dst, dst_port), rules)
}
...
/// The frame summary the egress verdict reads for a TCP connection to
/// `dst:port` — an Ethernet header carrying the minimum IPv4 header and a
/// 4-byte L4 header, with EtherType, IHL, protocol, destination address and
/// destination port set at the offsets `summarize` extracts them from.
#[must_use]
fn tcp_frame_summary(dst: Ipv4Addr, dst_port: u16) -> FrameSummary {
