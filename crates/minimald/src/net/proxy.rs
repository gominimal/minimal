...
//!
//! [`Router`] is that routing core, factored so every proxy surface shares one
//! `Host:`-header → registry → target lookup. The host-side
...
/// the upstream PTask. Generic over the client transport (the egress proxy
/// serves plain `TcpStream`s).
