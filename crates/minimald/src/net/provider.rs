...
mod tests {
    ...
    async fn host_ip_box_resolves_through_answerer() {
        ...
        let plan = network_for(NetworkMode::HostNet, &native, "s", None, deny_all.clone())
            .plan()
            .await
            .unwrap();
        ...
        // decided.
        let open = network_for(NetworkMode::HostNet, &native, "s", None, None)
            .plan()
            .await
            .unwrap();
        ...
        let plan = network_for(NetworkMode::HostNet, &native, "s", None, listed)
            .plan()
            .await
            .unwrap();
        assert_eq!(plan.resolver(), &Resolver::Host);
        ...
        let vm_backed = counting_switch();
        let plan = network_for(NetworkMode::HostNet, &vm_backed, "s", None, deny_all)
            .plan()
            .await
            .unwrap();
        assert_eq!(
    ...
    async fn unenforcing_native_host_runs_host_ip_box_unenforced() {
        ...
        for policy in [
            ...
            let net = network_for(NetworkMode::HostNet, &native, "s", None, policy);
