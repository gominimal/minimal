...
async fn unpack_workspace_files(
    ...
    unpack_validated(reader, paths.working.as_utf8_path().as_std_path())
        .await
    ...
    R: tokio::io::AsyncRead + Unpin + Send,
{
    ...
    while let Some(entry) = entries.next().await {
        ...
        let raw = entry
            ...
            .into_owned();
        if raw.as_os_str() == "pax_global_header" {
            continue;
        }

        // An entry path escaping the tree is fatal: a file writing to `../x`
        // has no benign form.
        let safe = common::archive::normalize_within_root(&raw)
            .ok_or_else(|| format!("entry escapes the workspace: {}", raw.display()))?;
...
mod tests {
    ...
    /// A workspace tarball carrying an escaping link plus one ordinary file,
    /// so tests can assert the link is dropped *and* the rest survives.
    fn tar_with_escaping_link(target: &std::path::Path, symlink: bool) -> Vec<u8> {
        let mut buf = Vec::new();
        let mut builder = tar::Builder::new(&mut buf);

        let mut link = tar::Header::new_gnu();
        link.set_entry_type(if symlink {
    ...
    async fn unpack_validated_skips_links_escaping_the_workspace() {
        for symlink in [true, false] {
            ...
            let tar = tar_with_escaping_link(&outside, symlink);
            unpack_validated(std::io::Cursor::new(tar), &dest)
                .await
                .expect("upload should succeed with the link skipped");

            assert!(
                dest.join("link").symlink_metadata().is_err(),
                "escaping link was planted (symlink={symlink})",
            );
            assert_eq!(
                std::fs::read_to_string(dest.join("keep.txt")).unwrap(),
                "keep",
                "the rest of the upload should still land",
            );
        }
    }

    /// An entry path escaping the workspace is fatal, not skipped — a regular
    /// file writing to `../x` has no benign interpretation.
    #[tokio::test]
    async fn unpack_validated_rejects_traversing_entry_paths() {
        let tmp = tempfile::tempdir().unwrap();
        let dest = tmp.path().join("ws");
        std::fs::create_dir(&dest).unwrap();
        ...
        let mut buf = Vec::new();
        {
            ...
            // The tar *writer* refuses `..` via set_path, so write the raw
            // name field, exactly as a hand-crafted archive would.
            let name = b"../evil.txt";
        ...
        let err = unpack_validated(std::io::Cursor::new(buf), &dest)
            .await
            .unwrap_err();
