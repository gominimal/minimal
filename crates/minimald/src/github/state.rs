...
/// The registered GitHub App's slug, per the PRD's authentication model.
///
/// **Must equal the App's real slug on GitHub.** It builds the installation URL
/// surfaced by [`github::Error::AppNotInstalled`] (spec R1.5), which the client
/// prints for the user to click; a stale value sends them to a 404 while every
/// other part of the flow keeps working, so the mistake is invisible from
/// inside the daemon.
///
/// Not read from [`GithubConfig`] because the config carries no App-identity
/// field — the App is one global identity, not per-deployment configuration.
pub(super) const APP_SLUG: &str = "gominimal";
