...
- Library crates: typed error enum via thiserror. `#[derive(Debug, thiserror::Error)]`; `#[error("...")]` for Display, `#[from]` for From. Variants are distinct failure modes. Keep `#[non_exhaustive]` on public error enums. See [Rust error handling](error-handling.md).
