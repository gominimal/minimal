...
`min init` inspects the source tree, detects the stack (Go, Rust, pnpm, npm, Bun, Deno, Python, Gradle, Make, and more), and writes a `minimal.toml`:
...
Minimal builds or fetches the declared packages, composes them into a sandbox rooted at your project directory, and drops you into a shell inside it. The `[session]` tools are on your `PATH`, and nothing touches the host. Type `exit` to leave; the session stays up, so you can `min attach` back to it later.
