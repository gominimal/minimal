...
- Network enforcement, box names in DNS, port publication, forwarding, ingress, and the VM egress filter: the networking spec, `docs/specs/18-spec-box-networking` (NET), and its epic gominimal/minimal#1437. This spec accepts and passes the `[network]` section (BOX-076 to BOX-081); NET enforces it. For NET-013, NET-015 and the draft GWI-005, a box "exists" while it is in the `running` state, and for NET-012 and GWI-005 it is "destroyed" or "removed" when it leaves `running` by a stop, an exit or a reap; a stopped or exited box is therefore neither kept running nor answers its name. BOX-005 resolves a name for the CLI, but NET's name registration covers the running box only (see Design reasoning and the Open questions).
...
**NET's "exists" read against BOX's five states.** NET-012, NET-013, NET-015 and the draft GWI-005 speak of a box that "exists", is "destroyed" or "is removed", written before a box was retained after it ends. BOX reads "exists" as the `running` state, and "destroyed" or "removed" as leaving `running` by a stop, an exit or a reap. NET-015 then keeps only a running box running, which agrees with BOX-013; NET-013 answers a name only while a running box holds it; NET-012's NXDOMAIN applies from the moment the box stops, exits or is reaped; and GWI-005 tears public exposure down at the same point, so nothing stays exposed with no box behind it. A stopped or exited box keeps its record (BOX-018) and its name for the CLI, where BOX-005 resolves it, but not in DNS; when a new running box reuses the name (BOX-004), NET registers that box. Reading "exists" as the record's lifetime was dropped because it makes NET-015 keep a stopped box running and NET-013 answer the name of a box with nothing behind it. NET and GWI are asked to confirm this reading in the open question below.
