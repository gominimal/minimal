...
A sandbox has one of three network modes: no network, the host network, or its
own IP address on the local switch. The mode is a property of the session, but
the code that starts an own-IP network is at the top of the daemon, in the
session layer. A new network option must pass through four layers of code
before it reaches the sandbox. The top layer also holds the rollback logic for
a switch attachment that the sandbox layer does not know about. Two different
paths start a sandbox process, and only one of the two paths applies the
network. Because of this, a task in an own-IP session gets the host network
instead of the network of its session. That is a silent increase of the network
access of the task, and it is the defect that makes this work necessary now.
After this change, one interface supplies the network to every sandbox, and the
sandbox layer controls the sequence and the rollback.

**Success:** A task and an interactive shell in the same own-IP session get the
same network mode, and only one module in the daemon reads the network mode.
...
**Roles:** the contributor who writes daemon code, the person who uses a
session, and the operator who runs the daemon.
...
- AS A person who uses a session I WANT my tasks to have the network of my
  session SO THAT a task cannot reach a host that my session cannot reach.
...
- **012-005** WHEN a task runs in a session, THE SYSTEM SHALL give the task the
  network mode of that session.
  tier:     T0
  verify:   `cargo nextest run -p minimald task_inherits_the_session_network_mode`
  - IF the daemon cannot attach the task to the switch, THEN THE SYSTEM SHALL
    stop the task with an error and SHALL keep the task off the host network.
    tier:   T0
    verify: `cargo nextest run -p minimald failed_task_attach_does_not_use_host_net`
...
5. The task path calls the same function as the session path, so 012-005 costs
   one line instead of a second implementation.
...
5. Move the task path onto the provider function. This satisfies 012-005.
